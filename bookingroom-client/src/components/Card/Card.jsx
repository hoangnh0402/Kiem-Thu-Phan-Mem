...
const Card = ({ option, data, keys, deleteOption, revertOption, deleteFlag, callbackKeyWord }) => {
      ...
      <div className='card'>
        <div className='card-body'>
                              ...
                              if (value) {
                                const result = await dispatch(
                                  ...
                                  .then((originalPromiseResult) => {
                                    console.log(originalPromiseResult)
                                    if (originalPromiseResult.status === 'SUCCESS') {
                                      Swal.fire('Thêm sale thành công', '', 'success')
                                      setTimeout(() => {
                    ...
                    <li>
                      <a
                        ...
                        onClick={(e) => {
                          e.preventDefault()
                          Swal.fire({
                            title: 'Bạn có chắc chắn xóa khuyến mãi của tất cả các phòng đã chọn?',
                            text: '',
                            icon: 'warning',
                            showCancelButton: true,
          ...
          <div className='table-responsive'>
                ...
                {data &&
                  data.map((item, index) => (
                    <tr
                      ...
                      className={active.includes(index) && option === 'rooms' ? 'active' : ''}>
                      {option === 'rooms' && (
                        <td>
                          <input
                            type='checkbox'
                                      ...
                                      const result = await dispatch(
                                        ...
                                        .then((originalPromiseResult) => {
                                          ...
                                          if (originalPromiseResult.status === 'SUCCESS') {
                                            ...
                                            console.log(originalPromiseResult.message[item['id']])
                                            Swal.fire(
                                              originalPromiseResult.message[item['id']],
                                              '',
                                              'error',
                                            )
                                          }
            ...
            {data && data.length === 0 && (
              <div style={{ textAlign: 'center', padding: '12px' }}>no {option} data</div>
            )}
          </div>
        </div>
      </div>
